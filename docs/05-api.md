...
Проверяет актуальность ранее рассчитанного DeliveryOffer перед подтверждением или использованием его условий.

Endpoint используется как при первоначальном оформлении заказа перед оплатой, так и при согласовании альтернативного варианта доставки.
...
| `cancellationPending` | Признак того, что запрос на отмену принят, но результат отмены ещё не подтверждён |
...
Поле `cancellationPending` возвращается во всех успешных ответах `GET /deliveries/{deliveryId}`.

Значение `true` означает, что запрос на отмену принят, но окончательный результат ещё не установлен.

Значение `false` означает, что ожидающего запроса на отмену нет.

Признак формируется на основании сохранённого запроса отмены (`cancelRequestedAt`) и текущего результата операции.

После подтверждения отмены Delivery переходит в `CANCELLED`, а признак `cancellationPending` принимает значение `false`.
...
Пока результат отмены остаётся неопределённым, `cancellationPending` сохраняет значение `true`. Это позволяет клиенту получать актуальную информацию об ожидающей отмене через `GET /deliveries/{deliveryId}`.
